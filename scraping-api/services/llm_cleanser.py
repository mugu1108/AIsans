"""
LLM企業名クレンジングモジュール v4

v3からの変更点:
- _normalize_company_name() を大幅強化
  - 「のホームページ」「の公式サイト」等の接尾辞除去
  - パイプ以降のテキスト除去（LLMが残した場合の保険）
  - カッコ除去の正規表現を改善（半角・全角両対応）
- _is_invalid_company_name() を強化
  - 文章パターンの検出（「〜する合同会社」等）
  - 「なら」パターンの改善（末尾だけでなく中間も検出）
  - 文字数による追加チェック
- セーフティネットの実行を保証するログ追加
"""
